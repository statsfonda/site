...
La loi des grands nombres montre que, $\mathbb{P}$-presque sûrement, $F_n(t)$ converge vers $\mathbb{P}(X_i \leqslant t)=F(t)$. On peut étendre ce résultat simultanément à une quantité dénombrable de $t$ (par exemple, $\mathbb{Q}$) mais pas à tous. De plus, ce résultat ne dit pas si la *fonction* $F_n$ est proche de la fonction $F$, au sens de la norme uniforme par exemple. Le théorème suivant, parfois appelé *théorème fondamental de l'estimation*, confirme que c'est le cas au sens de la norme uniforme^[On rappelle que $|g|_\infty = \sup_{x\in\mathbb{R}}|g(x)|$.]. 

:::{#thm-glivenko}

## Théorème de Glivenko-Cantelli

$\mathbb{P}$-presque sûrement, $\lim_{n\to \infty}|F_n - F|_\infty = 0$. 
...
## Calculabilité et loi {#sec-gc-calcul}

Soit $(X_{(1)}, \dotsc, X_{(n)})$ l'échantillon trié en ordre croissant. Par convention, on pose $X_{(0)} = -\infty$ et $X_{(n+1)} = \infty$. La quantité $|F_n - F|_\infty$ est calculable grâce à la représentation suivante : 
$$|F_n - F|_\infty = \sup_{i\in \{0, \dotsc, n\}}\left|\frac{i}{n} - F(X_{(i)})\right| \vee \left| \frac{i}{n} - F(X_{(i+1)}-)\right|,  $${#eq-supf}
où $F(x-)$ est la limite à gauche de $F$ en $x$, c'est-à-dire la limite de $F(x-t)$ lorsque $t>0$, $t\to 0$. Ainsi, lorsque $F$ est continue, $F(X_{(i)}) = F(X_{(i+1)}-)$.
...
$$ \sup_{i\in \{0, \dotsc, n\}}\left|\frac{i}{n} - U_{(i)}\right| \vee \left| \frac{i}{n} - U_{(i+1)}\right|$$
...
En choisissant par exemple $N = \lceil 2/t\rceil$, le terme entre parenthèses est plus grand que $t/2$, et la borne devient elle-même plus petite que $2e^{-2nt^2/4}$. 
Le terme @eq-uniongc est alors plus petit que $N 2e^{-nt^2/2}$, c'est-à-dire 
$$\mathbb{P}(|F_n - F|_\infty > t) \leqslant \frac{6}{t}e^{-nt^2/2}$${#eq-predkw}
où on a utilisé la borne très grossière $N \leqslant 3/t$. 

Si l'on choisit une suite $t_n$ qui tend vers 0, et telle que $\sum_n e^{-nt_n^2/2}/t_n < \infty$, alors le lemme de Borel-Cantelli permet de conclure : presque sûrement, à partir d'un certain rang, on a $|F - F_n|_\infty \leqslant t_n$, et donc $|F_n - F|_\infty \to 0$. N’importe quel choix de type $t_n = n^{0.5 - \varepsilon}$ pour $\varepsilon > 0$ convient, et donne même que $|F_n - F|_\infty = O(n^{0.5-\varepsilon})$. 
...
Il faut comparer ce résultat avec @eq-predkw, dans lequel la borne est effectivement décroissante en $nt^2$, mais avec un préfacteur $1/t$ polynomial. L'inégalité DKW donne une décroissance *exponentielle* en $nt^2$. 

[^1]: Prudence : il faut aussi que $N$ soit plus grand que 1, car sinon les quantiles n'ont pas de sens. Mais $\lceil 2/t\rceil$ vaut 1 uniquement lorsque $t$ est plus grand que 2 : dans ce cas, $\mathbb{P}(|F_n - F|_\infty > t) = 0$, puisque $|F_n - F|_\infty \leqslant |F|_\infty + |F_n|_\infty =2$. 