...
1. À quelle condition sur les variables $X,Z$ a-t-on $\hat{\bt}_X = \hat{\bt}$ ?
...
3. Même question lorsque $\varepsilon^1,\varepsilon^2$ n'ont pas la même variance mais que $\sigma_1,\sigma_2$ sont connus. 

:::

:::{#exr-covnonscal}

On se place dans un modèle linéaire *gaussien* de la forme $Y = X\bt + \varepsilon$, mais on suppose que les entrées de $\varepsilon_i$ ne sont plus iid, mais possèdent une covariance $\Sigma$ non scalaire.  On suppose que l'on connaît $\Sigma$. Montrer que $$\hat{\bt}_{\mathrm{MCG}}= (X^\top \Sigma^{-1} X)^{-1}X^\top \Sigma^{-1}Y$$
