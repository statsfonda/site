...
8. On observe un échantillon iid $(X_1, \dotsc, X_n)$ de loi $N(\mu, \sigma^2)$. On veut estimer le ratio de Sharpe $\zeta =\mu/\sigma$. Montrer que l'estimateur $\hat{\zeta}_n = \bar{X}_n / \hat{\sigma}_n$ est convergent. Est-ce qu’il a un biais ? [cf Examen 2025](/pdfs/exam_25.pdf). 
9. On observe un échantillon iid $(X_1, \dotsc, X_n)$ de loi $N(\mu, \sigma^2)$. Trouver un estimateur convergent de la kurtosis. Montrer qu’il est asymptotiquement normal.
...
2. On considère  l'estimateur $X_{(n)}= {\max}_{1\leq i \leq n}X_i$.  Déterminer les propriétés asymptotiques de ces estimateurs et les comparer. 
