...
7. Montrer la borne suivante sur les quantiles de loi gaussienne standard: $q_{1-\beta} < \sqrt{\ln\frac{1}{\beta\sqrt{2\pi}}}$ pour tout $\beta$ suffisamment proche de 0. 
...
$$\mathbb{P}(|X|>t) \leqslant \inf_{q \in \mathbb{N}}t^{-q}\mathbb{E}[|X|^q].$$
...
:::{#exr-gaussian-concentration}

## Concentration gaussienne

1. Calculer $\mathbb{E}[e^{t X}]$ lorsque $X$ est une variable gaussienne. 
2. En déduire que si les $X_i$ sont des gaussiennes iid $N(\mu,\sigma^2)$, alors 
$$\mathbb{P}(|\bar{X}_n-\mu|>t)\leqslant 2 e^{-n t^2 / 2\sigma^2}.$$
