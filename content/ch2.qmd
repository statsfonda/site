...
:::{.proof}
Fixons une fonction test $\varphi$ continue à support compact, donc bornée par un certain $M$. Pour montrer la convergence en loi, il faut montrer que $\mathbb{E}[\varphi(X_n, Y_n) - \varphi(X,c)]$ tend vers zéro. L'intégrande est égal à la somme de $A = \varphi(X_n, Y_n) - \varphi(X_n, c)$ et de $B=\varphi(X_n, c) - \varphi(X, c)$. 
...
![Une petite gaussienne autour de 1.5 est transformée par une fonction $f \in \mathscr{C}^1$ en une petite gaussienne autour de $f(1.5)\approx 1.8$, de variance normalisée par $f'(1.5)$. ](/images/histogram.png){width=60%}

:::

::: {.content-visible when-format="html"} 

![Une petite gaussienne autour de 1.5 est transformée par une fonction $f \in \mathscr{C}^1$ en une petite gaussienne autour de $f(1.5)\approx 1.8$, de variance normalisée par $f'(1.5)$. ](/images/histogram.webp)
...
Soit $(X_n)$ une suite de variables aléatoires réelles telle que $\sqrt{n}(X_n - \alpha)$ converge en loi vers $N(0,\sigma^2)$. Pour toute fonction $g : \mathbb{R} \to \mathbb{R}$ dérivable en $\alpha$ (de dérivée non nulle en $\alpha$), la variable aléatoire
